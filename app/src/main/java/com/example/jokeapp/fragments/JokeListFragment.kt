...
//            val episodeIds = args.episodes?.map { url ->
//                url.substringAfterLast("/")
//            }.joinToString(",")
//
//            getJokesFromAPI(episodeIds)
//        }
//        }
//    }
//
...
//        if (args.name == getString(R.string.favorites)) {
...
class JokeListFragment : Fragment(), JokesAdapter.JokeClickListener, Callback<List<EpisodeResponse>> {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.getJokesButton.setOnClickListener {
            ...
            episodeIds?.let { ids -> getEpisodesFromAPI(ids) }
        }
    }

    private fun getEpisodesFromAPI(ids: String) {
        val apiCall = RetrofitClient.getClient().create(ApiService::class.java)
            .getJoke(ids)
        binding.pendingIndicator.visibility = View.VISIBLE
        binding.getJokesButton.isEnabled = false
    ...
    override fun onResponse(call: Call<List<EpisodeResponse>>, response: Response<List<EpisodeResponse>>) {
        binding.pendingIndicator.visibility = View.GONE
        binding.getJokesButton.isEnabled = true
        if (response.isSuccessful) {
            val episodesResponse = response.body()
            episodesResponse?.let {
                (binding.jokesRecyclerView.adapter as JokesAdapter).submitList(it)
                Snackbar.make(
                    binding.root,
                    getString(R.string.jokes_number_info, it.size),
                    Snackbar.LENGTH_LONG
                ).show()
            }
        } else {
    ...
    override fun onFailure(call: Call<List<EpisodeResponse>>, t: Throwable) {
        binding.pendingIndicator.visibility = View.GONE
        binding.getJokesButton.isEnabled = true
        Snackbar.make(
            binding.root,
    ...
    override fun onJokeClick(episode: EpisodeResponse) {
